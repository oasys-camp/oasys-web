// netlify/functions/generate-confirmation-with-ambassadors.js
// Generate confirmation document with Bombón & Peluche inspiration
...
exports.handler = async (event, context) => {
  ...
  try {
    ...
    // Generate HTML confirmation document with ambassador inspiration
    const html = generateAmbassadorConfirmationHTML(plaza);

    // Save document to Supabase Storage
    const filename = `confirmacion_plaza${plaza_number}_${plaza.pet_name?.toLowerCase().replace(/\s+/g, '_') || 'mascota'}_con_embajadores.html`;
    ...
    console.error('Error in generate-confirmation-with-ambassadors:', error);
    return {
      statusCode: 500,
...
function generateAmbassadorConfirmationHTML(plaza) {
...
<title>Plaza Nº ${plaza.plaza_number} · El Bosque Sagrado · Inspirado por Bombón & Peluche</title>
...
<style>
  * { margin:0; padding:0; box-sizing:border-box; }
  ...
  .ambassador-section { background:#f5f0e8; padding:24px 40px; margin:32px 40px; border-left:3px solid #c9a84c; }
  .ambassador-title { font-family:'IBM Plex Mono',monospace; font-size:11px; color:#1a472a; letter-spacing:0.15em; text-transform:uppercase; margin-bottom:12px; }
  .ambassador-names { font-size:20px; font-weight:300; color:#1a472a; margin-bottom:8px; }
  .ambassador-names em { font-style:italic; color:#c9a84c; }
  .ambassador-quote { font-size:14px; color:rgba(17,17,17,0.6); font-style:italic; line-height:1.6; margin-top:12px; }
    ...
    <div class="header">
      ...
      <div class="body">
        ...
        <div class="ambassador-section">
          <div class="ambassador-title">🌟 Inspirado por nuestros embajadores</div>
          <div class="ambassador-names">Bombón <em>&</em> Peluche</div>
          <p class="ambassador-quote">
            "Desde su consciencia animal, Bombón y Peluche nos invitan a hacer las cosas bien.
            Como embajadores fundadores (Plazas Semilla Nº 1 & 2), guían cada nuevo árbol
            que se planta en nuestro bosque sagrado."
          </p>
        </div>
      ...
      <div class="steps-section">
        ...
        <div class="step">
          ...
          <p>Tu árbol será guiado por el espíritu de Bombón y Peluche, nuestros embajadores fundadores.</p>
        </div>
      </div>

      <div class="footer">
        <p>Este documento es la confirmación oficial de tu plaza en El Bosque Sagrado.</p>
        <p>Inspirado por Bombón & Peluche, nuestros embajadores fundadores.</p>
